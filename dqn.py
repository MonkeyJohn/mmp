...
class DQNAgent(object):
    def __init__(
        ...
        self.q_net_parms_convs = q_net_parms[]
        self.epsilon = exploration_fraction
        self.epsilon_step = (exploration_fraction - exploration_final_eps) / max_timesteps
    ...
    def build_training_op(self, q_network_weights):
        ...
        tf.losses.huber_loss(labels=y,predictions= q_value, delta=1.0)

        optimizer = tf.train.RMSPropOptimizer(LEARNING_RATE, momentum=MOMENTUM, epsilon=MIN_GRAD)
        grads_update = optimizer.minimize(loss, var_list=q_network_weights)

        return actions, y, loss, grads_update
